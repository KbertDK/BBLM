...
export default async function SchedulePage({ searchParams }: Props) {
  ...
  // When no league selected: LIVE first, then rest
  ...
  // Group by round (league view) or flat list (all-leagues view)
  ...
  return (
    <div className="min-h-screen bg-bb-navy">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-bb-muted text-sm tracking-wide mt-1">
            {selectedLeague
              ...
              : 'Upcoming fixtures across all active leagues'}
          </p>
        </div>
        ...
        {/* Division filter pills — only when a league with multiple divisions is selected */}
        {leagueId && divisions.length > 1 && (
          <div className="mb-8 mt-3">
            <p className="text-xs font-heading tracking-widest uppercase text-bb-muted/50 mb-2">Division</p>
            <div className="flex flex-wrap gap-2">
              <Link
                ...
                className={clsx(
                  'px-3 py-1.5 text-xs font-heading tracking-wider uppercase rounded-sm border transition-colors',
                  !divisionId
                    ? 'bg-bb-gold/20 text-bb-gold border-bb-gold/40 font-bold'
                    : 'border-bb-border text-bb-muted hover:border-bb-gold/40 hover:text-bb-gold',
                )}
              >
                All Divisions
        ...
        {/* Spacer when no division filter */}
        {!(leagueId && divisions.length > 1) && <div className="mb-8" />}

        {/* ── League view: grouped by round ───────────────────────────── */}
        {leagueId && (
          <>
            {matches.length === 0 ? (
              ...
              <div className="flex flex-col gap-6">
                {Array.from(roundMap.entries()).map(([round, roundMatches]) => {
                  ...
                  return (
                    <div key={round} className="bg-bb-dark border border-bb-gold/20 rounded-sm overflow-hidden shadow-lg shadow-black/40">
                      {/* Round header */}
